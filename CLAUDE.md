...
**【最高优先级】以下规则必须严格执行，任何情况下不得违反：**

1. **任务开始前**，必须调用 Read 工具读取 `.learnings/SETUP_TODO.md`
2. **每完成一个子步骤**，必须调用 Write 工具在 `.learnings/SETUP_TODO.md` 中打上 `[x]`
3. **进入下一个阶段前**，必须调用 Read 工具检查 `.learnings/SETUP_TODO.md`，确认上一步已完成
4. **如果发现某步骤未完成**，必须先完成该步骤，严禁跳步
5. **进度日志**：每完成一个子步骤，在进度日志表格中记录时间、阶段、操作、结果
